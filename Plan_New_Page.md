...
| Step | File                                                                                                                                                                                                                                                                                         | Change                                                                                                                                                                               |
| ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1    | [prisma/schema.prisma](vscode-webview://1dls5mpijv5j9pilcnqlendccajfb4qi6fpbo6c354ggasrtbjrj/prisma/schema.prisma)                                                                                                                                                                              | Added `Page` model with `@@unique([column, row])` + unique `name`. `db push` + `generate` applied.                                                                         |
| 2    | [prisma/seed-pages.ts](vscode-webview://1dls5mpijv5j9pilcnqlendccajfb4qi6fpbo6c354ggasrtbjrj/prisma/seed-pages.ts)                                                                                                                                                                              | One-shot seeder.**32 pages** seeded into their original grid positions (the plan said "33" but the source layout actually has 32).                                             |
| 3    | [src/app/admin/_actions/webnews.ts](vscode-webview://1dls5mpijv5j9pilcnqlendccajfb4qi6fpbo6c354ggasrtbjrj/src/app/admin/_actions/webnews.ts)                                                                                                                                                    | Added `addPage` action with transactional shift-down (`row DESC`, increment one-by-one to avoid unique-key collisions), redirects to `/admin/webnews/{name},New,1/new-record`. |
| 4    | [src/app/admin/webnews/new-page/page.tsx](vscode-webview://1dls5mpijv5j9pilcnqlendccajfb4qi6fpbo6c354ggasrtbjrj/src/app/admin/webnews/new-page/page.tsx) + [AddPageForm.tsx](vscode-webview://1dls5mpijv5j9pilcnqlendccajfb4qi6fpbo6c354ggasrtbjrj/src/app/admin/webnews/new-page/AddPageForm.tsx) | New route with name/column/row form, field-level errors, Cancel link.                                                                                                                |
| 5    | [src/app/admin/webnews/newspages/page.tsx](vscode-webview://1dls5mpijv5j9pilcnqlendccajfb4qi6fpbo6c354ggasrtbjrj/src/app/admin/webnews/newspages/page.tsx)                                                                                                                                      | Fully data-driven: reads from `db.page`, computes `maxRow`, renders 5×maxRow grid with `<p>` fillers, shows **Add Page** button.                                        |
| 6    | `webNewsDeletePage`                                                                                                                                                                                                                                                                        | Now wraps `deleteMany(WebNews) + deleteMany(Page)` in a `$transaction`; redirects to `/admin/webnews/newspages`.                                                               |
| 7    | [src/app/admin/webnews/[id]/update-page/page.tsx](vscode-webview://1dls5mpijv5j9pilcnqlendccajfb4qi6fpbo6c354ggasrtbjrj/src/app/admin/webnews/%5Bid%5D/update-page/page.tsx)                                                                                                                    | Inline server action wraps WebNews rename +`Page.update({name})` in a `$transaction`.                                                                                            |

**Verification:**

* `npx tsc --noEmit` — clean for every file I touched (only pre-existing error is unrelated `Checkbox.tsx`).
* DB query confirms 32 pages seeded into their original (col, row) positions.

**Caveats to be aware of:**

* `npm run build` now fails on a **pre-existing** Next.js 15 typing issue in `(customerFacing)/products/[id]/purchase/page.ts` (`params` must be `Promise` in Next 15). Several other `[id]` routes likely need the same migration. Not in scope for this plan; happy to fix in a follow-up if you want.
* The `newspages` rendering was changed from `grid-rows-9` to a dynamic `repeat(maxRow, minmax(0, auto))`. Since current `maxRow=7`, the grid is now 7 rows tall instead of 9 — visually a touch tighter, but matches your "1–7" specification.

Recommend you start `npm run dev` and walk through the §10 manual-verification checklist (especially: insert into a full column, insert into a column with fillers, duplicate-name rejection, rename a page, delete a page).
